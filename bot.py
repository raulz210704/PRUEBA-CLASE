...
@bot.command()
async def cibi(ctx, date_string: str = None):
    if date_string is None:
        await ctx.message.delete()
        await ctx.send("Te ha faltado especificar la fecha y hora, el formato es este: ")
    
    else:
        format = "%d-%m/%H:%M"
        hola = datetime.strptime(date_string, format)
        print(hola)
